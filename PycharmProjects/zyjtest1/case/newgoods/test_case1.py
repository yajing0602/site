...
class TestNewGoods(unittest.TestCase):
    ...
    def test03_newschedule(self,schedulename):
        ...
        result2=self.teachermanager.create_course(token,"自动化课程00")
        courseid=self.teachermanager.get_courseid(result2)
        result=self.teachermanager.create_course_schedule(courseid,schedulename,token)
        expected=result.json()["msg"]
        self.assertEqual(expected,"success")
    ...
    def test04_creat_schedule_work(self):
        result1 = self.teachermanager.login_manager()
        token = self.teachermanager.get_token(result1)
        result2 = self.teachermanager.create_course(token,"自动化课程10")
        courseid = self.teachermanager.get_courseid(result2)
        result3= self.teachermanager.create_course_schedule(courseid,"course_6",token)
