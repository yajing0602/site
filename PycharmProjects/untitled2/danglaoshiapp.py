...
class TeacherApp():
    ...
    def login_app(self,username,password):
        ...
        datas={"deviceType":1,
               ...
               "versionName":"3.3.6",
               "versionCode":77}
        response=self.s.post(url,headers=self.header,json=datas)  #通过json格式接收参数
        response = self.s.post(url, headers=self.header,data=json.dumps(datas))  #通过json.dumps()函数是将字典转化为字符串
        return  response
    ...
    def cousrse_detail(self,siduid,couserid1):
        ...
        self.header["sessionId"]=sessionid
        self.header["userId"]=userid
        courseid=random.choice(couserid1)
